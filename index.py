...
GEOMETRIA = delaunay.Delaunay1V([[0,0],[a,0],[a,a],[0,a]],delaunay._strdelaunay(a=5),cbe=[],valorCBE=0,plot=True)
gdls = np.array(GEOMETRIA.gdls)
labels = np.linspace(0,gdls.shape[0]-1,gdls.shape[0]).astype(int)
plt.plot(gdls[:,0],gdls[:,1],'o',markersize=10,color='gray')
for p,l in zip(gdls,labels):
    plt.annotate(l,p,size=5,textcoords="offset points",xytext=(-0,-2.5),ha='center')
