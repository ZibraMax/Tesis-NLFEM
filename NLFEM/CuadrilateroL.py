...
class CuadrilateroL(integradorRectangular):
    def __init__(this,coords,gdl=None,gauss=4):
        if len(coords) == 4:
            ...
            this.psis = lambda z,n: np.array([
                [1/4*(1-z)*(1-n)],
                [1/4*(1+z)*(1-n)],
                [1/4*(1+z)*(1+n)],
                [1/4*(1-z)*(1+n)]])
            this.dzpsis = lambda z,n: np.array([
                [1/4*(n-1)],
                [-1/4*(n-1)],
                [1/4*(n+1)],
                [-1/4*(1+n)]])
            this.dnpsis = lambda z,n: np.array([
                [1/4*(z-1)],
                [-1/4*(z+1)],
                [1/4*(1+z)],
                [1/4*(1-z)]])
            ...
            this.psis = np.array([
                lambda z,n: 1/4*(1-z)*(1-n),
                lambda z,n: 1/4*(1+z)*(1-n),
                lambda z,n: 1/4*(1+z)*(1+n),
                lambda z,n: 1/4*(1-z)*(1+n)])
            this.dzpsis = np.array([
                lambda z,n: 1/4*(n-1),
                lambda z,n: -1/4*(n-1),
                lambda z,n: 1/4*(n+1),
                lambda z,n: -1/4*(1+n)])
            this.dnpsis = np.array([
                lambda z,n: 1/4*(z-1),
                lambda z,n: -1/4*(z+1),
                lambda z,n: 1/4*(1+z),
                lambda z,n: 1/4*(1-z)])
